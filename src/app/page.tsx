    ...
    <>
      <section className="relative h-[min(82vh,820px)] overflow-hidden bg-navy-deep text-paper">
        ...
        <div className="relative mx-auto flex h-full max-w-7xl flex-col justify-end px-6 pb-20">
          <span className="mb-4 inline-flex w-fit items-center gap-2 rounded-full border border-paper/20 bg-paper/5 px-3 py-1 text-xs uppercase tracking-wider text-paper/70 backdrop-blur">
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            <Link href="/globe" className="inline-flex items-center gap-2 rounded-full bg-orange px-5 py-2.5 text-sm font-medium text-navy-deep transition hover:bg-orange-soft">
              Enter the Atlas <ArrowRight size={16} />
            </Link>
            <Link href="/sources" className="inline-flex items-center gap-2 rounded-full border border-paper/30 px-5 py-2.5 text-sm font-medium text-paper transition hover:bg-paper/10">
              Browse 1,932 sources
            </Link>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-6 py-24">
        ...
        <div className="mt-12 grid gap-px bg-border md:grid-cols-2">
          {CAPABILITIES.map(({ icon: Icon, title, body, href }) => (
            <Link key={title} href={href} className="group bg-paper p-8 transition hover:bg-slate-100/70">
